...
try:
    flag_reranker = FlagReranker('BAAI/bge-reranker-large')
    reranker_status_msg = "✅ FlagReranker initialized successfully with model: BAAI/bge-reranker-large"
...
def init_clients():
    ...
    mdl_clip = None
    if torch:
        proc = CLIPProcessor.from_pretrained(SCI_CLIP_MODEL)
...
def ask_question(query, set_status=None):
    ...
    try:
        ...
        return f"Error: {e}",[],[],[],0.0,[],debug_logs,[]
    ...
    if not snips: 
        ...
        return f"No docs for '{query}'",[],[],[],0.0,[],debug_logs,[]
    t_rerank = time.time()
    if flag_reranker:
    ...
    img_scores_final=[]
    if proc and mdl:
        av=embed_text_clip(answer,proc,mdl)
        total_cand = len(cand)
    ...
    paper_refs = re.findall(r'paper\s+(\d{4}\.\d{4,5}v?\d*)', answer, re.IGNORECASE)
    ...
    for sec in split_sections(answer):
    ...
    # Only return the answer with the image inline, not as a separate images list
    return final,relevant_chunks,used_ids,[],thr,img_scores_final,debug_logs,debug_img_list
